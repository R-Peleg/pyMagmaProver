...
class EquationPathSolver:
    ...
    def try_solve(self,
        ...
        for eq in assumed_equations:
            if eq.lean_name is None:
                raise ValueError(f"Equation used for Lean proof has no hypothesis name: {eq}")
        ...
        # all_variable_expr.extend([Expression.from_str(v + ' ◇ ' + v) for v in all_variables])
